<body class="bg-[#C1CFA1] flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-md md:max-w-lg">
        ...
        <!-- Title -->
        <h2 class="text-center text-2xl font-bold text-[#4D5A32] mb-6">Log In untuk masuk ke mode admin</h2>
        ...
        <form action="<?= BASEURL ?>pageAdmin/submitAdmin" method="POST">
            ...
            <!-- Email -->
            <div class="mb-4">
                <label for="username" class="ml-1 block text-sm font-semibold text-[#4D5A32]">Username</label>
                <input name="username" id="username" placeholder="Masukkan username Anda..."
                    class="mt-1 block w-full px-3 py-2 border <?= empty($data['errors']['username']) ? 'border-gray-300' : 'border-red-400' ?> rounded-lg focus:outline-none focus:ring-[#C1CFA1] focus:border-[#C1CFA1]"
                    value=<?= $data['username'] ?? '' ?>>
